                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <?php if ($row->id != $current_user->id && $row->role != 'administrator') : ?>
                                                    <?php if ($row->status == 0) : ?>
                                                        <a href="<?= site_url('admin/user/setActive/' . $this->secure->encrypt_url($row->id)) ?>" class="btn btn-success btn-sm">
                                                            Aktifkan
                                                        </a>
                                                    <?php else : ?>
                                                        <a href="<?= site_url('admin/user/setNonactive/' . $this->secure->encrypt_url($row->id)) ?>" class="btn btn-warning btn-sm">
                                                            Nonaktifkan
                                                        </a>
                                                    <?php endif; ?>
                                                    <a onclick="deleteConfirm('<?= site_url('admin/user/delete/' . $this->secure->encrypt_url($row->id)) ?>')" href="#!" id="btn-delete" class="btn btn-datatable btn-icon btn-danger"><i data-feather="trash-2"></i></a>
                                                <?php endif; ?>
                                            </td>
                                        </tr>
                                    <?php
